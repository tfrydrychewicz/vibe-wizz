...
- `notes:update` triggers `scheduleEmbedding(id)` fire-and-forget after save (no await); runs L1 chunk embeddings + L2 note summary (require sqlite-vec + OpenAI key) and **NER entity detection** (requires only Anthropic key) in background
...
- `SettingsModal.vue` — settings modal (opened from sidebar Settings button); two password inputs: OpenAI API key (for embeddings) and Anthropic API key (for note summaries); both load on mount via `settings:get` and save via `settings:set`
...
- **Embedding pipeline** (`src/main/embedding/`): `chunker.ts` splits `body_plain` into ~1600-char sentence-bounded chunks with ~200-char overlap; `embedder.ts` lazy-initialises an OpenAI client (text-embedding-3-small, 1536d) keyed by API key from `settings`; `summarizer.ts` lazy-initialises an Anthropic client (claude-haiku-4-5-20251001) for L2 note summarization; `ner.ts` detects entity mentions via Claude Haiku (NER); `pipeline.ts` orchestrates fire-and-forget L1+L2+NER pipeline after every note save — L1 chunks embedded via OpenAI into `chunk_embeddings`, L2 Claude-generated note summary embedded via OpenAI into `summary_embeddings`, NER detects entity mentions and upserts `auto_detected` rows into `entity_mentions`; vec0 tables (`chunk_embeddings`, `summary_embeddings`, `cluster_embeddings`) store FLOAT[1536] embeddings as BLOBs; `note_chunks.id` (INTEGER PRIMARY KEY AUTOINCREMENT) = rowid in respective vec0 table; vec0 has NO ON DELETE CASCADE — pipeline deletes from vec0 tables before `note_chunks`; `isVecLoaded()` exported from `db/index.ts` — L1/L2 return early if false; L1 requires only `openai_api_key`; L2 additionally requires `anthropic_api_key`; NER requires only `anthropic_api_key` (runs independently of sqlite-vec)
