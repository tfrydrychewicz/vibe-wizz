...
- **Embedding pipeline** (`src/main/embedding/`): `chunker.ts` splits `body_plain` into ~1600-char sentence-bounded chunks with ~200-char overlap; `embedder.ts` lazy-initialises an OpenAI client (text-embedding-3-small, 1536d) keyed by API key from `settings`; `summarizer.ts` lazy-initialises an Anthropic client (claude-haiku-4-5-20251001) for L2 note summarization; `pipeline.ts` orchestrates fire-and-forget L1+L2 pipeline after every note save — L1 chunks embedded via OpenAI into `chunk_embeddings`, L2 Claude-generated note summary embedded via OpenAI into `summary_embeddings`; vec0 tables (`chunk_embeddings`, `summary_embeddings`, `cluster_embeddings`) store FLOAT[1536] embeddings as BLOBs; `note_chunks.id` (INTEGER PRIMARY KEY AUTOINCREMENT) = rowid in respective vec0 table; vec0 has NO ON DELETE CASCADE — pipeline deletes from vec0 tables before `note_chunks`; `isVecLoaded()` exported from `db/index.ts` — all embedding paths return early if false; L1 requires only `openai_api_key`; L2 additionally requires `anthropic_api_key`
