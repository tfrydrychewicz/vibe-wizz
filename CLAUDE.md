...
- `notes:delete` is a soft-delete (sets `archived_at`) and also clears `linked_note_id` on any `calendar_events` and `action_items` that referenced it; `notes:restore` clears `archived_at`; `notes:delete-forever` replaces all `[[noteLink]]` nodes in other notes' bodies with plain text of the note title, cleans up `note_relations`, then hard-deletes (FK `ON DELETE SET NULL` auto-clears `calendar_events.linked_note_id` and `action_items.linked_note_id`)
...
- `action-items:list` — `{ status?, source_note_id?, project_entity_id?, is_waiting_for?, someday? }` → `ActionItem[]` (with `source_note_title`, `linked_note_id`, `linked_note_title`, `assigned_entity_name`, `project_name`, `waiting_for_name` via JOINs); non-cancelled by default
- `action-items:get` — `{ id }` → `ActionItem | null`: full row with joins for a single item; used by `TaskDetailPanel`
- `action-items:get-by-linked-note` — `{ note_id }` → `ActionItem | null`: returns the task that has `linked_note_id = note_id`; used by `NoteEditor` to show the task context header for task notes
- `action-items:get-subtasks` — `{ parent_id }` → `ActionItem[]`: all non-cancelled sub-tasks of a parent
- `action-items:create` — `{ title, source_note_id?, linked_note_id?, assigned_entity_id?, due_date?, extraction_type?, confidence?, project_entity_id?, contexts?, energy_level?, is_waiting_for?, parent_id? }` → full row with joins
- `action-items:update` — `{ id, title?, status?, linked_note_id?, assigned_entity_id?, due_date?, project_entity_id?, contexts?, energy_level?, is_waiting_for?, waiting_for_entity_id?, someday?, parent_id? }` → `{ ok }`; sets/clears `completed_at` based on status; always stamps `updated_at` when any field changes; fires `action:updated` push event with `{ actionId }`
...
- `TaskDetailPanel.vue` — right-side detail panel for a single GTD task; mounted in `App.vue` as a fixed overlay; fields: title (auto-save on blur/Enter), status select, project search (entity dropdown filtered by `gtd_project_entity_type_id`), assignee search, due date, contexts (tag input), energy level (Low/Med/High toggle buttons with `aria-pressed`), waiting-for toggle + entity search, sub-task list with inline add (`SubTaskInput`); **Attached Note** section (create new or attach existing; `TaskNotePreview` with fixed 160px height, scrollable); source note chip; created/updated footer; subscribes to `action:updated` push for in-place refresh; all interactive fields have ARIA labels; `role="complementary"` on root; IPC: `action-items:get`, `action-items:update`, `action-items:get-subtasks`, `action-items:derive-attributes`
- `TaskCard.vue` — unified task card used in `ActionsView` and `TaskDetailPanel` sub-task list; props: `task: ActionItem`, `depth: number`, `showProject: boolean`, `showSourceNote: boolean`, `showLinkedNote: boolean`; emits `open-detail`, `status-changed`, `open-note`, `subtask-created`; renders GTD attribute chips inline; linked (attached) note chip + expandable `TaskNotePreview` (120px height, scrollable); status toggle checkbox; "Derive" button fires `action-items:derive-attributes` with spinner state
- `TaskNotePreview.vue` — reusable scrollable note content preview; props: `noteId`, `noteTitle`, `height` (default 140px); fetches via `notes:get`; renders `body_plain` in fixed-height overflow-y-auto container; Open button supports all 3 open modes
- Task notes: when a note is linked to a task (`action_items.linked_note_id`), `NoteEditor` shows a **task context header** (like meeting header): "Task Note" label, task title, project/due/status chips, "Open in Actions" button; created task notes use title `Task Note: {task title}`
