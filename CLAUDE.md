...
- `settings:get` — `{ key }` → stored value string or null; `settings:set` — `{ key, value }` → upserts; known keys: `openai_api_key`, `anthropic_api_key`, `calendar_slot_duration`, `attendee_entity_type_id`, `attendee_name_field`, `attendee_email_field`
...
- `chat:send` — `{ messages: {role, content}[], searchQuery?: string }` → `{ content: string, references: {id, title}[] }`: FTS5 search on the last user message (top 5 notes) → inject as context into Claude Sonnet system prompt → parse `[Note: "Title"]` citations from response → return structured references; returns graceful message if no `anthropic_api_key` configured
...
- `SettingsModal.vue` — settings modal (opened from sidebar Settings button); sections: AI/Embeddings (OpenAI + Anthropic keys), Calendar (slot duration + **Attendee Entity** config); Attendee Entity section: entity type dropdown (from `entity-types:list`) + Name Field + Email Field dropdowns (text/email fields from the selected type's schema; `__name__` = entity primary name column); saves via `settings:set` for `attendee_entity_type_id`, `attendee_name_field`, `attendee_email_field`
...
- **MeetingModal.vue** — create/edit meeting modal; fields: title, date, start/end time, attendees (add/remove chips), linked note (open in all 3 modes, unlink); two-step delete confirm; calls `calendar-events:create` or `calendar-events:update` on save; `calendar-events:delete` on delete; `CalendarEvent` type exported from here (id: number, external_id, title, start_at, end_at, attendees JSON string, linked_note_id, linked_note_title); attendee items type: `{ name, email, entity_id? }`; on mount loads `attendee_entity_type_id/name_field/email_field` settings — if all set, attendee input switches to entity search mode (calls `entities:search` filtered by type, `entities:get` on selection to resolve name/email fields); if not set, shows free-form name+email inputs
...
- `calendar-events:update` — `{ id, title?, start_at?, end_at?, attendees?, linked_note_id? }` → `{ ok }`; dynamic SET clause
