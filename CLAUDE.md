...
- **NoteEditor.vue** transcription additions: "Start Transcription" / "Stop" button in the meeting context header (only shown when `linkedCalendarEvent` is set); `startTranscription()` invokes `transcription:start`, branches on `audioFormat` response: `'pcm'` → `ScriptProcessorNode` (16kHz Int16 for ElevenLabs), `'webm'` → `MediaRecorder` 250ms chunks (for Deepgram), `'none'` → no renderer audio setup (Swift captures directly); collapsible live transcript panel shows real-time output; on `transcription:complete` push event, reloads note body; auto-starts if `pendingAutoStartNoteId` matches on mount
...
- Transcription IPC (registered in `src/main/transcription/session.ts`):
  - `transcription:start` (invoke) — `{ noteId, eventId }` → `{ ok, audioFormat?, error? }`; routes by `transcription_model` setting: `'elevenlabs'` → ElevenLabs WS → `audioFormat:'pcm'`; `'deepgram'` → Deepgram WS → `audioFormat:'webm'`; `'macos'` → Swift binary → `audioFormat:'none'`
